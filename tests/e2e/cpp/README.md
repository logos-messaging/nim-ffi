...
codegen is usable from a real C++ consumer. They drive the `timer` example
through its auto-generated `timer.hpp` bindings (constructor, sync method,
...
- `CMakeLists.txt` — `add_subdirectory`s `examples/timer/cpp_bindings`, which
  compiles `libtimer` and exposes the `timer_headers` INTERFACE target.
