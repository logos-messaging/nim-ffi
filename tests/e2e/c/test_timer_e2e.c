 ...
 * out with the pump or after a wait on the wake handle.
...
/* Handlers run on the thread that pumps, which is main here: no atomics. */
typedef struct {
    int echo_hits;
    int scheduled_hits;
...
static void test_event(MyTimerCtx* ctx) {
    ...
    while (sink.echo_hits == 0 && now_ms() < deadline) {
        int rc = my_timer_ctx_pump_once(ctx, 50, &handlers);
...
static void test_ignored_event(MyTimerCtx* ctx) {
    ...
    while (rc == NIMFFI_RET_TIMEOUT && now_ms() < deadline) {
        rc = my_timer_ctx_pump_once(ctx, 50, NULL);
    }
    assert(rc == NIMFFI_RET_OK);
}
