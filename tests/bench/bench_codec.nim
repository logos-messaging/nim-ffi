## Nim-level microbenchmark comparing the two FFI wire-format codecs:
##
##   - CBOR path:  `cborEncode(req)` + `cborDecode(bytes, T)` — self-describing
##                 bytes over `seq[byte]`, the codec the `ffiMode=cbor` shim
##                 uses on every boundary crossing.
##   - cwire path: `cwirePack(dst, src)` + `cwireUnpack(src)` + `cwireFree(dst)`
##                 — flat C-struct shared-memory packing, the path the
##                 `ffiMode=raw` shim uses on every boundary crossing.
##
...
## chronos work). For the full FFI round-trip cost see the C/C++ e2e
## benchmarks under `tests/bench/{cpp_cbor,c_raw}`.
##
## Must be compiled with `-d:ffiMode=raw` so the cwire companion procs
## (`*_CWire`, `cwirePack`, `cwireUnpack`, `cwireFree`) are emitted by the
## {.ffi.} type pragma. `genBindings()` at file end flushes any types whose
## cwire emission was deferred.
...
proc reportTiming(label: string, perOp: float, iters: int) =
  ...
  echo "  " & padded & " " & formatFloat(perOp, ffDecimal, 2) &
    " ns/op  (" & $iters & " iters)"
...
when isMainModule:
  const modeTag = static(meta.ffiMode)
  echo &"nim-ffi codec microbench  (ffiMode={modeTag})  {now()}"
  echo "──────────────────────────────────────────────────────────────────"
  echo ""
  benchEchoSimple()
