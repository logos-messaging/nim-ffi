  ...
  while true:
    ...
    if rc == RET_OK:
      ...
      if msg.kind == MsgReply:
        ...
        # The ctor's reply carries no value of its own; every other one is the
        # `n` it was sent.
        const NoValue = [@[CborNullByte], @[byte 0x60]]
        if msg.retCode == RET_OK and payload notin NoValue:
...
proc churnLoop(w: ptr Worker) =
  ...
  for _ in 0 ..< w.iters:
    ...
    var reqId = 0'u64
    if sendRequestToFFIThread(
      ctx, SoakWorkReq.ffiNewReq(nil, nil, 1), ctx.currentGeneration(), addr reqId
    )
        .isErr():
      w.failures.atomicInc()
    if reqId != 0:
