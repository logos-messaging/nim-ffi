...
suite "a context token is safe for a host to hold":
  test "no token lands in the first page":
    ...
    var pool: FFIContextPool[ChurnLib]
    for _ in 0 ..< 4:
...
proc churnLoop(c: ptr Churn) =
  for _ in 0 ..< c.iters:
    ...
    var reply: CallbackData
    initCallbackData(reply)
    let started = getMonoTime()
    if sendRequestToFFIThread(ctx, ChurnWork.ffiNewReq(testCallback, addr reply, 1))
        .isOk():
      # The condvar wait, not the polling one: this measures the library, and a
      # 10 ms poll would hide the latency this case is here to catch.
      waitCallback(reply)
...
when not defined(gcRefc):
  ## Skipped under refc: a heap belongs to one thread there, and the event
  ## registry's table is allocated by whichever thread creates the context and
  ## cleared by the FFI thread. Creating contexts from several threads trips over
  ## that before it reaches anything this suite is about. The registry goes away
  ## when events move to `poll`.
  suite "many threads share the pool":
    test "a slot changing owner fails no create, no recycle and no reply":
      ...
      # handover goes wrong. Eight is enough to keep every slot moving and stays
      # clear of a recycle stall that is still open at sixteen; raise it with
      # FFI_CHURN_WORKERS to reproduce a handover fault that needs more pressure.
      let workers = parseInt(getEnv("FFI_CHURN_WORKERS", "8"))
      var pool = cast[ptr FFIContextPool[ChurnLib]](allocShared0(
        sizeof(FFIContextPool[ChurnLib])
      ))
      defer:
