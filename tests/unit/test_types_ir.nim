...
suite "parseFFIType: structs and handles":
  ...
  test "a name listed as a handle becomes ftHandle":
    let handles = toHashSet(["Timer"])
    let t = parseFFIType("Timer", handles)
    check t.kind == ftHandle
    check t.name == "Timer"

  test "without the handle set the same name is a plain struct":
    check parseFFIType("Timer").kind == ftStruct
