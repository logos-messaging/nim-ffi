...
suite "generateCHeader":
  setup:
    let types = @[
      FFITypeMeta(name: "TimerConfig",
                  fields: @[FFIFieldMeta(name: "name", typeName: "string")]),
      FFITypeMeta(name: "EchoRequest",
                  fields: @[
                    FFIFieldMeta(name: "message", typeName: "string"),
                    FFIFieldMeta(name: "delayMs", typeName: "int"),
                  ]),
      FFITypeMeta(name: "EchoResponse",
                  fields: @[
                    FFIFieldMeta(name: "echoed", typeName: "string"),
                  ]),
      FFITypeMeta(name: "ComplexRequest",
                  fields: @[
    ...
    let procs = @[
      FFIProcMeta(procName: "timer_create",   kind: FFIKind.CTOR,
                  libName: "timer", libTypeName: "Timer",
                  extraParams: @[
                    FFIParamMeta(name: "config", typeName: "TimerConfig"),
                  ],
                  returnTypeName: "Timer"),
      FFIProcMeta(procName: "timer_version",  kind: FFIKind.FFI,
                  libName: "timer", libTypeName: "Timer",
                  extraParams: @[],
                  returnTypeName: "string"),
      FFIProcMeta(procName: "timer_echo",     kind: FFIKind.FFI,
                  libName: "timer", libTypeName: "Timer",
                  extraParams: @[
                    FFIParamMeta(name: "req", typeName: "EchoRequest"),
                  ],
                  returnTypeName: "EchoResponse"),
      FFIProcMeta(procName: "timer_destroy",  kind: FFIKind.DTOR,
                  libName: "timer", libTypeName: "Timer",
                  extraParams: @[],
                  returnTypeName: ""),
    ]
  ...
  test "extern function signatures match the C-target ABI":
    ...
    check header.contains(
      "int timer_echo(void* ctx, const TimerEchoReq* req, " &
        "FfiCallback cb, void* user_data);"
    )
...
suite "generateCCMakeLists":
  test "substitutes lib name and source-path placeholders":
    ...
    check cm.contains("-d:targetLang=c")
    check cm.contains("CMAKE_CURRENT_SOURCE_DIR}/../timer.nim")
