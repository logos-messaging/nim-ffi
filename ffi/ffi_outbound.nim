...
## single-consumer, the message counter and the message last handed out.
## chronos-free, because the poller is a host thread.
...
type
  ...
  FFIOutbound* = object
    ...
    heldMsg*: array[2, NimFfiMsg]
      ...
      # them, used in turn: a slot outlives its owners, and the next owner's
      # first message must not land in the one the last owner is still reading.
    heldMsgSlot*: int
