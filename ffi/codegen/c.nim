...
proc ensureCTypeIR(reg: var CTypeReg, t: FFIType): tuple[cType: string, owns: bool] =
  ...
  of ftBytes:
    ("NimFfiBytes", true)
  of ftSeq:
    let (elemC, _) = ensureCTypeIR(reg, t.elem)
    ...
    (name, true)
  of ftOpt:
    let (elemC, elemOwns) = ensureCTypeIR(reg, t.elem)
...
proc ensureCType(reg: var CTypeReg, nimType: string): tuple[cType: string, owns: bool] =
  ensureCTypeIR(reg, parseFFIType(nimType))
...
func paramByValue(nimType: string, ridesAsPtr: bool): bool =
  ...
  if ridesAsPtr:
    return true
  parseFFIType(nimType).kind in {ftScalar, ftStr, ftPtr, ftHandle}
