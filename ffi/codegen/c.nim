...
## CBOR, no serialization, no third-party deps.
##
## Counterpart to ffi/codegen/cpp.nim, which generates a C++ wrapper that
## speaks CBOR-on-the-wire to the same Nim library. Both run in-process;
## the difference is the wire format (raw structs here, CBOR there), not
## the deployment model. The two paths are independent.
