...
proc generateCppHeader*(
  ...
  for ctor in ctors:
    ...
    lines.add("        if (!ffi_ctx_->pumpThread_.joinable()) {")
    lines.add(
      "            return $1::err(\"could not start the event pump thread\");" %
        [createRet]
    )
    lines.add("        }")
  ...
  lines.add("    std::thread pumpThread_;")
  lines.add("    explicit $1(void* p, std::chrono::milliseconds t)" % [ctxTypeName])
  lines.add("        : ptr_(p), timeout_(t), pump_(std::make_shared<NimFfiPump>()) {")
  lines.add(
    "        pumpThread_ = NimFfiPump::start(pump_, &$1_poll, ptr_, &$2::dispatchEvent_);" %
      [libName, ctxTypeName]
  )
  lines.add("    }")
  lines.add("};")
