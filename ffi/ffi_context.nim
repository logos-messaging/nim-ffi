...
type FFIContext*[T] = object
  ...
  reqQueue: FFIRequestQueue # lock-free MPSC ingress from foreign threads
...
proc deinitContextResources*[T](ctx: ptr FFIContext[T]): Result[void, string] =
  ## Mirror of `initContextResources`. Threads MUST be joined first (so the
  ## FFI thread has drained every queued request); fields are nil'd after close
  ## so re-init on the same slot is safe. `reqQueue`'s stub is inline, so it
  ## needs no teardown — re-init rebinds head/tail on the next createFFIContext.
