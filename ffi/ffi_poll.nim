...
proc fill(
  ...
  if data.isNil() or len <= 0:
    data = cast[pointer](emptyPayload)
  var msgSeq = seq
  if msgSeq == 0:
    msgSeq = outb.nextSeq()
  outb.heldMsg = NimFfiMsg(
...
proc checkLiveness[T](
  ...
  ## True when `msg` was filled with a liveness report. Each report latches once per episode.
