...
##
## When the in-process ABI is active, for every {.ffi.} object type T and
## every per-proc Req/Resp we generate a parallel `T_CWire` Nim object
## whose field layout matches the C struct emitted by `ffi/codegen/c.nim`:
##   - `string` → `cstring`
