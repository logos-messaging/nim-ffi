...
proc buildLibReadyGuard*(
  ...
  ## Rejects a request that reached the FFI thread without a `{.ffiCtor.}` having
  ## stored the library.
  ##
  ## Only for `ref` library types. When no ctor has run, `myLib` points at the
  ## FFI thread's default-valued fallback (see `processQueue`); for an `object`
  ## that is a usable zero value and callers may legitimately rely on it, but for
  ## a `ref` it is `nil`, so the user body would deref nil on its first field
  ## access. Emitted inside the handler, i.e. on the FFI thread behind any queued
  ## ctor, so a host that calls without awaiting the create callback still works.
  quote:
