...
public:
    ...
    static Result<std::unique_ptr<EchoCtx>> create(const EchoConfig& config, std::chrono::milliseconds timeout = std::chrono::seconds{30}) {
        ...
        if (!ffi_ctx_->pumpThread_.joinable()) {
            return Result<std::unique_ptr<EchoCtx>>::err("could not start the event pump thread");
        }
...
private:
    ...
    explicit EchoCtx(void* p, std::chrono::milliseconds t)
        ...
        pumpThread_ = NimFfiPump::start(pump_, &echo_poll, ptr_, &EchoCtx::dispatchEvent_);
    }
};
