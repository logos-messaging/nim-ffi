...
- `src/api.rs`: High-level async/sync wrapper around the FFI. `MyTimerMessage` lists everything the library sends (events, liveness, closed); one dispatch thread per context takes the messages out with `my_timer_poll` and runs the `add_*_listener` handlers
