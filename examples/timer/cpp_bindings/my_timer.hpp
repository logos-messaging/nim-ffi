/* ============================================================
 * AUTO-GENERATED by nim-ffi v0.1.3  (ffiMode=cbor, ffiLang=cpp). DO NOT EDIT.
 *
 *   Source:     ../timer.nim
...
inline CborError encode_cbor(CborEncoder& e, const BytesPayload& v) {
    CborEncoder m;
    CborError err = cbor_encoder_create_map(&e, &m, 1);
    if (err) return err;
...
inline CborError decode_cbor(CborValue& it, BytesPayload& v) {
    if (!cbor_value_is_map(&it)) return CborErrorImproperValue;
    CborValue field;
    CborError err;
...
struct MyTimerBytesEchoReq {
    BytesPayload blob;
};
inline CborError encode_cbor(CborEncoder& e, const MyTimerBytesEchoReq& v) {
    CborEncoder m;
    CborError err = cbor_encoder_create_map(&e, &m, 1);
    if (err) return err;
    err = cbor_encode_text_stringz(&m, "blob"); if (err) return err;
    err = encode_cbor(m, v.blob);              if (err) return err;
    return cbor_encoder_close_container(&e, &m);
}
inline CborError decode_cbor(CborValue& it, MyTimerBytesEchoReq& v) {
    if (!cbor_value_is_map(&it)) return CborErrorImproperValue;
    CborValue field;
    CborError err;
    err = cbor_value_map_find_value(&it, "blob", &field); if (err) return err;
    if (!cbor_value_is_valid(&field)) return CborErrorImproperValue;
    err = decode_cbor(field, v.blob); if (err) return err;
    return cbor_value_advance(&it);
}
...
int my_timer_bytes_echo(void* ctx, FFICallback callback, void* user_data, const uint8_t* req_cbor, size_t req_cbor_len);
...
public:
    ...
    BytesPayload bytes_echo(const BytesPayload& blob) const {
        const auto ffi_req_ = MyTimerBytesEchoReq{blob};
        const auto ffi_req_bytes_ = encodeCborFFI(ffi_req_);
        const auto ffi_raw_ = ffi_call_([&](FFICallback cb, void* ud) {
            return my_timer_bytes_echo(ptr_, cb, ud, ffi_req_bytes_.data(), ffi_req_bytes_.size());
        }, timeout_);
        return decodeCborFFI<BytesPayload>(ffi_raw_);
    }

    std::future<BytesPayload> bytes_echoAsync(const BytesPayload& blob) const {
        return std::async(std::launch::async, [this, blob]() { return this->bytes_echo(blob); });
    }
