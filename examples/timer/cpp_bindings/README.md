...
- `my_timer.hpp`: High-level C++ class (`MyTimerCtx`) wrapping the FFI interface. Each context runs one pump thread that takes the library's messages out of `my_timer_poll` and calls the listeners (`addOn<Event>Listener`, `addNotRespondingListener`, `addRespondingListener`, `addClosedListener`)
